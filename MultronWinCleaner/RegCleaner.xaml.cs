...
namespace MultronWinCleaner
{
    public partial class RegCleaner : Window
    {
        ...
        private async void CleanButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var issuesToClean = _issues.ToList();
            List<RegistryIssue> failedDeletions = new();

            int total = issuesToClean.Count;
            int processed = 0;
            ...
            {
                for (int i = 0; i < issuesToClean.Count; i++)
                {
                    var issue = issuesToClean[i];
                    bool success = await DeleteRegistryKey(issue.Path);
                    ...
                    if (i % 10 == 0 || i == issuesToClean.Count - 1)
                    {
            ...
            Dispatcher.Invoke(() =>
            {
                ...
                {
                    MessageBox.Show($"{failedDeletions.Count} entries could not be deleted.", "Some Issues", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
            });
        }
