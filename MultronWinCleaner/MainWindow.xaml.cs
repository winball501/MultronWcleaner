...
namespace Multron_Win_Cleaner
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private DateTime lastArcUpdate = DateTime.MinValue;
        private System.Windows.Shapes.Path _arcPath;
        private void UpdateArc(double percent)
        {
            if (percent < 100 && (DateTime.Now - lastArcUpdate).TotalMilliseconds < 50)
                return;

            lastArcUpdate = DateTime.Now;

            double angle = 360 * (percent / 100);
            double radius = progressBar1.ActualWidth / 2 - 7.5;
            var center = new Point(progressBar1.ActualWidth / 2, progressBar1.ActualHeight / 2);
                ...
                try
                {
                    ...
                    await main.Dispatcher.InvokeAsync(async () =>
                    {
                        ...
                        foreach (var group in groupedByPath)
                        {
                            ...
                            {
                                FileName = $"File to delete={item.file}={Formatsize(item.size)}",
                                SizeBytes = item.size,
                                IsChecked = true
                            }).ToList();
            ...
            private async Task LoadMoreItemsForListBox_ByGroup(ListBox listbox, string groupid)
            {
                ...
                await main.Dispatcher.InvokeAsync(() =>
                {
                    nextBatch.ForEach(originalBox =>
                    {
                        ...
                        Debug.WriteLine($"🟢 Eklendi: {newBox.Name}, Content: {newBox.Content}");
                    });
                });

                Debug.WriteLine($"✅ {nextBatch.Count} checkbox eklendi. ListBox: '{listbox.Name}', Grup: '{groupid}'");
            }
        ...
        private void CheckBox_Checked(object sender, RoutedEventArgs e)
        {
            ...
            if (checkBox != null)
            {
                ...
                if(content.Contains("logscan"))
                {
                    ...
                    database.Insert(0,name + "=" + path + "=" + "logscan");
