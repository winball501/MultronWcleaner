...
namespace Multron_Win_Cleaner
{
    ...
    public partial class MainWindow : Window
    {
       ...
       public void UpdateArc(double percent)
        ...
        public class AutoClean
        {
         ...
         out FILETIME userTime);
            ...
            public static float GetCpuUsage()
            {
                ...
                ulong idle1 = FileTimeToUInt64(idleTime1);
                ulong kernel1 = FileTimeToUInt64(kernelTime1);
                ulong user1 = FileTimeToUInt64(userTime1);

                ulong idle2 = FileTimeToUInt64(idleTime2);
                ulong kernel2 = FileTimeToUInt64(kernelTime2);
                ulong user2 = FileTimeToUInt64(userTime2);

                ulong idleDelta = idle2 - idle1;
                ulong kernelDelta = kernel2 - kernel1;
                ulong userDelta = user2 - user1;
                ...
                float cpuUsage = (1.0f - ((float)idleDelta / total)) * 100f;
                return cpuUsage;
            }
        ...
        public static bool CheckBatteryLevelAbove(int minPercentage)
        {
            ...
            return batteryPercent >= minPercentage;
        }
        ...
        public class LoadLockedFiles
        {
            ...
            public async Task CheckWhoUsesMultipleAsync(IEnumerable<string> paths, IProgress<(int current, int total)>? progress = null)
            {
                ...
                await Parallel.ForEachAsync(pathList, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount }, async (path, _) =>
                {
            ...
            public async Task run()
            {
                ...
                await main.Dispatcher.InvokeAsync(() =>
                {
                    var allProcessesFlat = new List<LockedProcessViewModel>();
                    var addedKeys = new HashSet<string>();

                    foreach (var group in main.LockedFileGroups)
                    {
                        foreach (var proc in group.Processes)
                        {
                            FileInfo info = new FileInfo(group.FilePath);
                            string filesize = main.formatsize(info.Length);
                            string key = proc.DisplayName + "|" + group.FilePath;

                            if (!addedKeys.Contains(key))
                            {
                                addedKeys.Add(key);

                                allProcessesFlat.Add(new LockedProcessViewModel
                                {
                                    FilePath = group.FilePath + " " + filesize,
                                    FilePathWithoutSize = group.FilePath,
                                    DisplayName = proc.DisplayName,
                                    IsChecked = proc.IsChecked,
                                    OnCheckedChanged = (model, isChecked) =>
                                    {
                                        if (isChecked)
                                            main.paths.Add(model.FilePathWithoutSize + "=" + proc.Id);
                                        else
                                            main.paths.Remove(model.FilePathWithoutSize + "=" + proc.Id);
                                    }
                                });
                            }
                        }
                    ...
                    main._allLockedFileGroups = allProcessesFlat;

                    main.paths.Clear();
                    main.LockedProcesses.Clear();
                    foreach (var proc in main._allLockedFileGroups.Take(main._pageSize))
                        main.LockedProcesses.Add(proc);

                   main. _itemsLoaded = main.LockedProcesses.Count;
                    main.groupedProcesses = new CollectionViewSource
                    {
                        Source = main.LockedProcesses
                    };

                    main.groupedProcesses.GroupDescriptions.Add(new PropertyGroupDescription(nameof(LockedProcessViewModel.FilePath)));

                    var view = main.groupedProcesses.View;

                    string searchText = (main.SearchBox.Text ?? "").Trim().ToLower();
                    ...
                    else
                    {
                        view.Filter = item =>
                        {
                            if (item is LockedProcessViewModel proc)
                            {
                                return proc.FilePathWithoutSize?.ToLower().Contains(searchText) == true;
                            }
                            return false;
                        };
                    }
