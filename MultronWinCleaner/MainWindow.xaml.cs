...
namespace Multron_Win_Cleaner
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public async Task shredder(string dosya)
        {
            ...
            await Task.Run(async () =>
            {
                ...
                {
                    nstate = 1;
                    wfc = 8 * new FileInfo(dosya).Length;
                    string fname = new FileInfo(dosya).Name;
                    long fsize = new FileInfo(dosya).Length;
               
                    await System.IO.File.WriteAllBytesAsync(dosya, getrandombytes(1000000));

                    using (FileStream fs = new FileStream(dosya, FileMode.Open, FileAccess.Write))
                    {
                        yazar = new BinaryWriter(fs);
                        ...
                        while (wd < wfc)
                        {
                            ...
                            {
                                fs.Position = 0;
                                await fs.WriteAsync(getrandombytes(1000000), 0, 1000000);
                                cwd = 0;
                            }
                            await fs.WriteAsync(getrandombytes(1000000), 0, 1000000);
                            wd += 1000000;
                            ...
                            if(done == 0)
                            {
                                ...
                                {
                                    label1_Copy.Content = "Shredding: " + dosya + " | %" + Math.Round(((double)wd / (double)wfc) * 100, 0).ToString();
                                });
                            }
                        ...
                        if (fileinfo.Exists)
                        {
                            try
                            {
                                ...
                                if (main.excludedfiles.Contains(file))
                                {
                                    ...
                                    await main.Dispatcher.InvokeAsync(async () => {
                                        if (main.Shred.IsChecked == true && whousef.WhoIsLocking(file).Count == 0)
                                        {
                                            await main.shredder(file);
                                        }
                                    });
