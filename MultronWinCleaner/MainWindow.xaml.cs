...
namespace Multron_Win_Cleaner
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public class Scan
        {
            ...
            private readonly Dictionary<TextBlock, StackPanel> panelsByHeader = new();
            ...
            public async void run()
            {
                try
                {
                    ...
                    { 
                        ...
                        main.checkboxes2.Clear();
                        main.logfiles.Clear();
                        panelsByHeader.Clear();
                    });

                    long totalEntries = main.database.Count();
                    long processed = 0;
                    main.scanstatus = 2;
                    main.onclean = 1;
                    ...
                    foreach (var entry in main.database)
                    {
                        ...
                        try
                        {
                            string name = main.stringtokenizer(entry, "=", 0);
                            string path = main.stringtokenizer(entry, "=", 1);

                            bool isLogScan = entry.EndsWith("logscan");
                            TextBlock header = await CreateHeaderAndExpander(name, isLogScan);

                            if (isLogScan)
                                await ScanCDirectoryAsync(path, name, header);
                            else if (System.IO.File.Exists(path))
                                await AddFileSafely(header, path);
                            else
                                await ScanDirectoryAsync(path, name, header);
                        }
                        catch (Exception ex)
                        {
                            Debug.WriteLine($"[WARN] Skipping entry '{entry}': {ex.Message}");
                        }

                        processed++;
                        double pct = (double)processed / totalEntries * 100;
                        await main.Dispatcher.InvokeAsync(() =>
                        {
                            main.progressBar1.Value = pct;
                            main.UpdateArc(pct, main.progressBar1.Maximum);
                        });

                        await ShowStatus("Scanning: " + entry);
                    }

                    await main.Dispatcher.InvokeAsync(() =>
                    {
                        string result = (main.autoclean == 1 ? "Auto scan completed!" : "Scan completed!")
                            + " + " + formatsize(totalsize) + " total scanned. " + DateTime.Now;
                        main.label1_Copy.Text = result;
                ...
                catch (Exception ex)
                {
                    ...
                    await main.Dispatcher.InvokeAsync(() =>
                    {
                        main.label1_Copy.Text = "Error during scan: " + ex.Message;
                        main.label1_Copy.Foreground = Brushes.Red;
                        main.buttonStartScan.IsEnabled = true;
                    });
                }
            }

            private async Task<TextBlock> CreateHeaderAndExpander(string name, bool isLogScan)
            {
                ...
                await main.Dispatcher.InvokeAsync(() =>
                {
                    ...
                    main.wrapPanelDirectories.Children.Add(header);
                    ...
                    panelsByHeader[header] = panel;
                    main.wrapPanelDirectories.Children.Add(expander);
                });
                return header;
            }

            private async Task ScanDirectoryAsync(string dir, string name, TextBlock header)
            {
                ...
                {
                    Debug.WriteLine($"[WARN] Cannot enumerate files in '{dir}': {ex.Message}");
                    return;
                }
                ...
                foreach (var file in files)
                {
                    ...
                    try
                    {
                        ...
                        totalSz += sz;
                        totalsize += sz;
                        await AddFileSafely(header, file, sz);
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine($"[WARN] Skipping file '{file}': {ex.Message}");
                    }
                }

                await main.Dispatcher.InvokeAsync(() =>
                {
                    header.Text = $"Completed: {name} ({formatsize(totalSz)})";
                });
            }

            public async Task ScanCDirectoryAsync(string directory, string name, TextBlock header)
            {
                if (!Directory.Exists(directory))
                    return;

                string[] exts = new[] { ".log", ".etl", ".dmp", ".trace", ".tmp", ".temp", ".bak", ".swp" };
                IEnumerable<string> files;
                try
                {
                    files = Directory.GetFiles(directory, "*", SearchOption.AllDirectories)
                                     .Where(f => exts.Any(e => f.EndsWith(e, StringComparison.OrdinalIgnoreCase))
                                                 && !main.settings.excludedfiles.Contains(f));
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"[WARN] Cannot enumerate C directory '{directory}': {ex.Message}");
                    return;
                }
                ...
                foreach (var file in files)
                {
                    ...
                    try
                    {
                        ...
                        await AddFileSafely(header, file, info.Length);
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine($"[WARN] Skipping log file '{file}': {ex.Message}");
                    }
                }

                await main.Dispatcher.InvokeAsync(() =>
                {
                    header.Text = $"Completed log scan: {name} ({formatsize(size)})";
                });
            }

            private async Task AddFileSafely(TextBlock header, string file, long size = -1)
            {
                try
                {
                    await main.Dispatcher.InvokeAsync(() =>
                    {
                        if (!panelsByHeader.TryGetValue(header, out var panel))
                            return;

                        string displaySize = size >= 0 ? formatsize(size) : formatsize(new FileInfo(file).Length);
                        var cb = new CheckBox
                        {
                            Content = $"{file}=({displaySize})",
                            Margin = new Thickness(5, 2, 5, 2),
                            IsChecked = true,
                            Foreground = main.brush,
                            ToolTip = file
                        };
                        cb.Checked += main.CheckBox2_Checked;
                        cb.Unchecked += main.CheckBox2_Unchecked;

                        panel.Children.Add(cb);
                        main.checkboxes2.Add(cb);
                        main.checkboxes.Add(cb);
                    });
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"[WARN] UI add failed for '{file}': {ex.Message}");
                }
            }
            ...
            private async Task ShowStatus(string msg)
            {
                try
                {
                    await main.Dispatcher.InvokeAsync(() => main.label1_Copy.Text = msg);
                }
                catch { }
            }
            ...
            {
                double s = bytes;
                if (s >= 1 << 30) return $"{s / (1 << 30):0.##} GB";
                if (s >= 1 << 20) return $"{s / (1 << 20):0.##} MB";
                if (s >= 1 << 10) return $"{s / (1 << 10):0.##} KB";
                return $"{s} bytes";
            }
        }
        ...
        private void CheckBox2_Checked(object sender, RoutedEventArgs e)
        {
            ...
            string file = stringtokenizer(checkBox.Content.ToString(), "=", 0);
        ...
        public class Clean
        {
            ...
            public async void run()
            {
                ...
                await UpdateStatusColor(Brushes.Blue);
                ...
                if (main.logfiles.Any())
                {
                    ...
                    foreach (string file in main.logfiles)
                    {
                        ...
                        if (!System.IO.File.Exists(file)) { await ShowStatus($"File not found: {file}"); continue; }

                        if (main.settings.excludedfiles.Contains(file))
                        {
                            await ShowStatus($"Skipping: {file}");
                            continue;
                        }
                        ...
                        {
                            await ShowStatus($"Cannot delete: {file} - {ex.Message}");
                        }
                ...
                foreach (string item in main.database)
                {
                    if (main.cancelclean == 2) break;
                    ...
                    try
                    {
                        if (System.IO.File.Exists(path))
                        {
                            ...
                            {
                                await ShowStatus($"Skipping: {path}");
                            }
                        }
                        ...
                        {
                            await ShowStatus($"Not found: {path}");
                        }
                    ...
                    {
                        await ShowStatus($"Error deleting {path}: {ex.Message}");
                    }
            ...
            private async Task RecursiveClean(string path, HashSet<string> excluded)
            {
                string[] files = await Task.Run(() =>
                {
                    ...
                    {
                        Console.WriteLine($"GetFiles error in '{path}': {ex.Message}");
                ...
                string[] dirs = await Task.Run(() =>
                {
                    ...
                    {
                        Console.WriteLine($"GetDirectories error in '{path}': {ex.Message}");
                        return Array.Empty<string>();
                    }
                });
                ...
                foreach (var file in files)
                {
                    ...
                    await MaybeShowStatusAsync($"Deleting file: {file}");
                    ...
                    if (!excluded.Contains(file))
                    {
                        ...
                        {
                            await ShowStatusSafe($"Cannot delete file: {file} - {result.Item3}");
                        }
                    }
                }
                ...
                foreach (var dir in dirs)
                {
                    ...
                    if (!result.Item1)
                    {
                        await ShowStatusSafe($"Cannot delete directory: {dir} - {result.Item2}");
                    }
                }
            }
 

            private async Task ShowStatusSafe(string text)
            {
                await main.Dispatcher.InvokeAsync(() => ShowStatus(text));
            }

            private async Task MaybeShowStatusAsync(string text)
            {
              
                if ((DateTime.Now - lastStatusUpdate).TotalMilliseconds > 1000)
                {
                    lastStatusUpdate = DateTime.Now;
                    await ShowStatusSafe(text);
                }
            }
            private async Task FinalizeCleaning()
            {
                ...
                await main.Dispatcher.InvokeAsync(() =>
                {
                    ...
                    main.autoclean = 0;
                    main.onclean = 0;
                    main.buttonStartScan.Content = "Reset";
                    main.buttonStartScan.IsEnabled = true;
            ...
            private async Task UpdateProgress(int current, int total)
            {
                ...
                await main.Dispatcher.InvokeAsync(() =>
                {
                    ...
                    main.UpdateArc(percent, main.progressBar1.Maximum);
                });
            }
            ...
            private async Task UpdateStatusColor(Brush brush)
            {
                await main.Dispatcher.InvokeAsync(() =>
                {
