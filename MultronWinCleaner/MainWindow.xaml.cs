    ...
    public partial class MainWindow : Window
    {
        ...
        public class Scan
        {
                ...
                try
                {
                    ...
                    foreach (string directory in main.database)
                    {
                        ...
                        await main.Dispatcher.InvokeAsync(() =>
                        {
                            ...
                            {
                                Text = directory.EndsWith("logscan") ? $"Process: {name}" : $"Scanning: {name}",
                        ...
                        else
                        {
                            ...
                            {
                                totalsize += new FileInfo(directory).Length;
                            }
                            else
                            {
                    ...
                    await main.Dispatcher.InvokeAsync(() =>
                    {
                        ...
                        {
                            string resultText = (main.autoclean == 1 ? "Auto scan completed!" : "Scan completed!") +
                                                " + " + formatsize(totalsize) + "  Useless file found! " + DateTime.Now;

                            main.label1_Copy.Text = resultText;
            ...
            public async Task ScanCDirectoryAsync(string directory, string name, TextBlock directorytextblock)
            {
                try
                {
                    long totalsize = GetCDirectorySize(directory);
            ...
            private long GetCDirectorySize(string path)
            {
                long size = 0;
                ...
                try
                {
                    ...
                    foreach (string file in files)
                    {
                        ...
                        HashSet<string> logExtensions = new() { ".log", ".etl", ".dmp", ".trace", ".tmp", ".temp", ".bak", ".swp" };
                        if (logExtensions.Any(ext => file.EndsWith(ext, StringComparison.OrdinalIgnoreCase)))
                        {
                ...
                {
                    Debug.WriteLine($"Access denied to {path}: {ex.Message}");
                }
                catch (Exception ex)
                {
            ...
            private void AddFileToUI(string file, long size)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    checkBox.Checked += main.CheckBox2_Checked;
                    checkBox.Unchecked += main.CheckBox2_Unchecked;

                    main.checkboxes2.Add(checkBox);
                    main.checkboxes.Add(checkBox);
                    ...
                    if (main.created == 0)
                    {
                        ...
                        main.expanders.Add(expander);
                        main.wrapPanelDirectories.Children.Add(expander);

                        main.groupBoxContent = stackPanel;
                        main.expander = expander;
                    }
                    else
                    {
                        main.groupBoxContent?.Children.Add(checkBox);
                    }
                });
            }
            ...
            public async Task ScanDirectoryAsync(string directory, string name, TextBlock directoryTextBlock)
            {
                try
                {
                    long totalsize = await Task.Run(() => GetDirectorySize(directory));
                    this.totalsize += totalsize;
                    string sizeFormatted = formatsize(totalsize);

                    await main.Dispatcher.InvokeAsync(() =>
                    {
                        directoryTextBlock.Text = $"Completed: {name} {sizeFormatted}";
                    });
                }
                catch (Exception ex)
                {
            ...
            private long GetDirectorySize(string path)
{
    ...
    {
        Debug.WriteLine($"Error calculating size of {path}: {ex.Message}");
    }
