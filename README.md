...
Scan and remove junk files from 250+ supported applications, constantly updated (the database.txt file lives in a separate repo and can be extended both by the developer and by the community via commits/PRs)
...
Locked File Detection — Files locked by other processes are detected in real time, showing the locking process name/PID and full file path, with retry or force-delete options.

Scan Details — Each file's creation date, age in days, last modified date, and size are shown before cleanup.
